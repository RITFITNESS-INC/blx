...
/* The official TR2 480x480 knob demo exposes its rear serial connector as UART1. */
#define GRF_TENSION_UART_PORT 1
#endif
...
int grf_tension_app_init(void)
{
    ...
    /* The V3/V5 startup handshake requires device status to be read first.
       The controller sends the safe stop frame after the first valid response,
       or as a delayed fallback if the board does not answer within 400 ms. */
    if (tension_controller_poll_telemetry(&motor_controller) != TENSION_CTRL_OK) {
        grf_drv_uart_close(motor_uart);
        motor_uart = NULL;
        return -1;
    }
