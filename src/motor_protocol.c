    ...
    motor_telemetry_t *telemetry)
{
    ...
    /* V3/V5 feedback CHK is documented as unreliable and may cover only CMD
       + the first four payload bytes. Framing and the fixed 21-byte length are
       authoritative; calculate partial and nominal full XOR for diagnostics. */
    full_checksum = motor_protocol_checksum(frame + 1u, 18u);
    legacy_checksum = motor_protocol_checksum(frame + 1u, 5u);
    ...
    telemetry->feedback_checksum_received = frame[19];
    telemetry->feedback_checksum_matches_full = frame[19] == full_checksum;
    telemetry->feedback_checksum_matches_legacy = frame[19] == legacy_checksum;
    return MOTOR_OK;
}
