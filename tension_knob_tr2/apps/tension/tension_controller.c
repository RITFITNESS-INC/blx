...
static tension_ctrl_result_t send_active_frame(tension_controller_t *controller)
{
    ...
    case TENSION_MODE_BASE:
        result = motor_protocol_build_base(
            controller->settings.concentric_force_dN,
            controller->settings.eccentric_force_dN,
            true,
            frame,
            sizeof(frame),
            &written);
    ...
    case TENSION_MODE_SPRING:
        result = motor_protocol_build_spring(
            ...
            controller->settings.spring_peak_force_dN,
            frame,
            sizeof(frame),
            &written);
        break;
    case TENSION_MODE_BAND:
        result = motor_protocol_build_band(
            ...
            controller->settings.band_peak_force_dN,
    ...
    case TENSION_MODE_ISOKINETIC:
        result = motor_protocol_build_isokinetic(
            ...
            controller->settings.isokinetic_slope_dN_per_m_s,
            frame,
            sizeof(frame),
            &written);
        break;
    case TENSION_MODE_VIBRATION:
        result = motor_protocol_build_vibration(
            controller->settings.vibration_amplitude_dN,
    ...
    case TENSION_MODE_INERTIA:
        result = motor_protocol_build_inertia(
            controller->settings.inertia_breakout_force_dN,
            frame,
            sizeof(frame),
            &written);
        break;
    ...
    const tension_controller_io_t *io)
{
    ...
    controller->settings.concentric_force_dN = 490;
    controller->settings.eccentric_force_dN = 490;
    controller->settings.spring_travel_mm = 100;
    controller->settings.spring_peak_force_dN = 50;
    controller->settings.band_rest_length_mm = 100;
    controller->settings.band_maximum_length_mm = 500;
    controller->settings.band_peak_force_dN = 50;
    controller->settings.band_curve_factor = 1u;
    controller->settings.isokinetic_speed_mm_s = 100;
    controller->settings.isokinetic_slope_dN_per_m_s = 100;
    controller->settings.vibration_amplitude_dN = 50;
    controller->settings.vibration_frequency_hz = 10;
    controller->settings.inertia_breakout_force_dN = 50;
    ...
    const motor_telemetry_t *telemetry)
{
    ...
    if (controller->state == TENSION_STATE_DISCONNECTED) {
...
void tension_controller_tick(tension_controller_t *controller, uint32_t elapsed_ms)
{
    ...
    /* Active connection handshake: the motor board also waits for a command
       before it starts answering. While disconnected, periodically emit a
       zero-force base frame (safe: no output, SSR off) so the board can lock
       on and begin replying with telemetry. control_elapsed_ms is unused in
       the DISCONNECTED state, so reuse it as the handshake timer. */
    if (controller->state == TENSION_STATE_DISCONNECTED) {
        controller->control_elapsed_ms = saturating_add_u32(
            controller->control_elapsed_ms,
            elapsed_ms);
        if (controller->control_elapsed_ms >= TENSION_CONNECT_HANDSHAKE_PERIOD_MS) {
    ...
    if (controller->state == TENSION_STATE_RUNNING &&
        controller->control_elapsed_ms >= TENSION_CONTROL_PERIOD_MS) {
        controller->control_elapsed_ms %= TENSION_CONTROL_PERIOD_MS;
    ...
    bool accelerated)
{
    ...
    case TENSION_MODE_BASE:
        value = controller->selected_parameter == TENSION_PARAM_PRIMARY
            ...
            : &controller->settings.eccentric_force_dN;
        step = 10;
        maximum = TENSION_UI_MAX_FORCE_DN;
        break;
    case TENSION_MODE_SPRING:
        if (controller->selected_parameter == TENSION_PARAM_PRIMARY) {
            ...
            value = &controller->settings.spring_peak_force_dN;
            step = 10;
            maximum = TENSION_UI_MAX_FORCE_DN;
        }
        break;
    case TENSION_MODE_BAND:
        if (controller->selected_parameter == TENSION_PARAM_PRIMARY) {
            ...
            value = &controller->settings.band_peak_force_dN;
            step = 10;
            maximum = TENSION_UI_MAX_FORCE_DN;
        } else {
    ...
    case TENSION_MODE_ISOKINETIC:
        if (controller->selected_parameter == TENSION_PARAM_PRIMARY) {
            ...
            value = &controller->settings.isokinetic_slope_dN_per_m_s;
            step = 10;
            maximum = 4000;
        }
        break;
    case TENSION_MODE_VIBRATION:
        if (controller->selected_parameter == TENSION_PARAM_PRIMARY) {
            value = &controller->settings.vibration_amplitude_dN;
            step = 10;
            maximum = TENSION_UI_MAX_FORCE_DN;
        } else {
        ...
        break;
    case TENSION_MODE_INERTIA:
        value = &controller->settings.inertia_breakout_force_dN;
        step = 10;
        maximum = TENSION_UI_MAX_FORCE_DN;
        break;
    default:
...
tension_ctrl_result_t tension_controller_long_press(tension_controller_t *controller)
{
    ...
    if (!controller->telemetry_valid || controller->state == TENSION_STATE_FAULT) {
        return TENSION_CTRL_ERROR_STATE;
    }
